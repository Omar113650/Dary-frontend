...
export default function OwnerRoute({ children }: { children?: ReactNode }) {
  const { isAuthenticated, isLoading, isOwner, logout, role } = useAuth();
      ...
      >
        ...
        >
          ...
          <h2 style={{ color: '#0B2A4A', fontSize: '1.35rem', marginBottom: '0.75rem', fontWeight: 700 }}>
            {locale === 'ar' ? 'لوحة تحكم أصحاب العقارات' : 'Owner Dashboard'}
          </h2>
          <p style={{ color: '#64748B', fontSize: '0.95rem', lineHeight: 1.6, marginBottom: '1.5rem' }}>
            {locale === 'ar'
              ? `أنت مسجل الدخول بدور "${role || 'غير محدد'}". لوحة التحكم هذه مخصصة لحسابات أصحاب العقارات والمؤجرين فقط.`
              : `You are logged in with role "${role || 'unspecified'}". This dashboard is reserved for property owners only.`}
          </p>
